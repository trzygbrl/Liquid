...
- **Our wedge:** we're the only one running symptoms through a full navigation pipeline: specialty, then sub-specialty, then HMO verification, then a ranked doctor list, all before a human ever has to guess.
...
|---|---|---|
| **Innovation & Creativity** | 20% | Full symptom to sub-specialty to HMO to ranked-doctor pipeline (Section 8.1), not single-step search; HMO intelligence layer (8.3) that suggests alternatives on mismatch, not just filters |
...
2. Intake flow (multi-step): demographics, then HMO selection, then symptom free-text, then follow-up branching questions
...
**Symptoms, then AI Symptom Understanding, then Clinical Specialty Mapping, then Sub-specialty Identification, then HMO Verification, then Doctor Ranking, then Booking**
...
- **After:** your app's AI match result screen showing specialty, then sub-specialty, then HMO-verified, ranked doctor in a few taps.
...
- [ ] Patient types a realistic symptom description, then walk through all five pipeline stages out loud as they happen on screen (8.1) *(Innovation, AI)*
- [ ] A separate demo input triggers the emergency safety gate, then shows the calm interstitial instead of a booking flow *(AI, UX)*
