...
1. **Protect the main branch.** On GitHub: Settings, then Branches, then Add branch protection rule for `main`. Enable "Require a pull request before merging" and "Require at least 1 approval." This makes it physically impossible for anyone to push broken code straight to main, even by accident.
...
Treat "task done" as one sequence: commit, then push, then append the `BUILD_LOG.md` entry, then open the PR. Doing all four together means nothing gets forgotten, and Vercel's preview URL gives even non-technical teammates a way to verify a PR actually works. They can click the link instead of needing to read the code.
...
5. Commit and push:
   ...
   git push
   ```
6. Add all 5 team members as collaborators on the GitHub repo (Settings, then Collaborators, or make it an org repo if you have a GitHub org for the team).
...
Get a "hello world" version of the full pipeline (frontend, then backend, then database) live on a real URL before building any real feature. This surfaces integration pain on day 1, not day 10.
...
3. **Create a Supabase project:**
   ...
   - Once created, go to Project Settings, then API and copy the **Project URL** and **anon public key**. You'll need both.
...
Demographics, then HMO selection, then free-text symptoms, then up to 2 branching follow-ups.
...
### Task 3.2: AI symptom to sub-specialty matching integration (Owner: D)
...
**End of Phase 4 checkpoint:** the full loop works. Symptom, then match, then doctor list, then booking, then doctor sees it in their dashboard.
...
- **Before/after visual:** a mockup or screenshot-style comparison. "Before" showing a cluttered generic Google search, "after" showing your AI match result screen (specialty, then sub-specialty, then HMO-verified, ranked doctor). This can be a simple slide graphic, doesn't need to be built in the app.
...
Structure per PRD Section 13: problem statement, then positioning line, then live demo (walking through all 5 pipeline stages), then differentiation (name NowServing/MyDoktor.ph/KonsultaMD/Doctor Anywhere explicitly, per Section 3's objection-handling), then before/after visual + roadmap slide (PRD Section 11), then financial framing (PRD Section 9), then close on the tagline "The Right Doctor. The First Time."
